...
'''
new_import = '''from django.shortcuts import render, redirect, get_object_or_404
from django.contrib.auth import login, logout, authenticate
from django.contrib.auth.decorators import login_required
from django.contrib import messages
from django.views.decorators.http import require_POST
...
old_models = 'from .models import User, Profile\n'
new_models = 'from .models import User, Profile, Guardian, GuardianLinkRequest, GuardianLinkRequestLog\n'
if old_models in text:
    text = text.replace(old_models, new_models, 1)
else:
    print('models import line not found')
...
if not match:
    raise SystemExit('register_tutor block not found')

replace = '''def register_tutor(request):
...
text = text[:match.start()] + replace + text[match.end()-len('def login_view(request):'):]
